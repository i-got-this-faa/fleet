...
Run two or more parallel design agents with the design-sketch task and the Phase A grounding artifacts. Pass `references/runner-prompt.md` as each agent's prompt. Each agent produces a design package shaped per `references/rationale-template.md`: the caller's usage written first, then the type sketch, function signatures, module map, and prose rationale derived from it.

Use the configured architect runners. If no runner list exists, use two runs on the parent model. Keep the fan-out small.
...
4. Return to Phase B and run the parallel design agents again.
