...
- For one-way-door design decisions, run the **architect** skill with diverse, isolated, opinionated candidates and a read-only judge. Skip it for mechanical work whose shape is already concrete. Repeating parallel design work over a settled design is over-engineering (the **laziness-protocol** principle skill).
