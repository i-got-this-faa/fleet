...
- Use read-only explorer agents. Use configured models when the host supports explicit selection.
...
- Use one read-only explainer agent. Use a configured model when the host supports explicit selection.
...
After the explanation is complete, spawn one architectural critic per configured reviewer. If no list exists, use two critics on the parent model.

For each critic:
- Use read-only agent tasks. Pass an explicit model only when the host supports it.
