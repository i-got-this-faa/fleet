---
name: no-comments
description: "Review comments in a diff, fix valid findings, and encode repeated constraints in structure."
---

# No comments

Review the diff for comments that hide unclear code or stale claims. Fix valid findings.

Comments can defend a useful contract, but they can also hide a poor shape. Check the code and the caller before keeping one.
...
1. Use one read-only agent task for a fresh comment-focused review when the host supports agent tasks. Pass the scope and the diff. If no task tool is available, perform the review yourself.
