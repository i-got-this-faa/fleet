...
- Prefer the host's normal agent task type. Do not use a built-in planning task type that ignores this skill.
- Use the configured role model when the host supports it. Otherwise use the parent model.
...
If a phase creates or edits a skill, the phase instructs the implementer to use the **skill-creator** skill (the host's built-in for authoring SKILL.md files).
...
- Browser / Electron / Web UIs: the `control-ui` skill from the `installed control skills` plugin.
- CLIs and TUIs: the `control-cli` skill from the `installed control skills` plugin.
...
- the host's built-in **babysit** skill after opening the PR.
