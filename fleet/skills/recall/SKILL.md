...
Transcripts live in the active agent host's workspace-scoped history directory. Use the path supplied by the host. Do not infer a path from a product-specific layout.

1. Classify, then route. One specific prior chat to resume is the `session-pickup` playbook, not this. Turning habits into a durable skill is a separate skill-authoring task. A human-readable summary of your work is a different task. Recall loads working context across recent chats before you act. If the user already gave you a full state capsule (paths, branch, the change), use it and skip the mining.
