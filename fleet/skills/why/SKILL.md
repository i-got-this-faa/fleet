...
If the target is vague ("why do we do it this way?" with no clear referent), make your best guess from conversation context (open files, recent edits, and what was just discussed). State your interpretation briefly so the user can redirect if needed, then proceed.
...
Before spawning investigators, list the tools and connectors that the current host exposes. Use the host's tool map when present. Do not assume a connector exists because another product supports it.
...
Subagent config (each): use a read-only agent task when the needed tools are available. Use an explicit model only when the host supports it. Investigators must not write files.
...
Spawn one investigator per category that has a matching connector. Each owns exactly one tool or connector.
...
Spawn one synthesis agent task. Use the configured synthesis model or the parent model. It may use available connectors to spot-check citations, but it must not write files.
